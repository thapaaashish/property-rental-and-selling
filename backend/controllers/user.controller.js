...
export const getUserDetailsForPublic = async (req, res, next) => {
  try {
    const user = await User.findById(req.params.id).select(
      "fullname avatar bio role address city province zipCode createdAt email"
    );
    if (!user) {
      return next(errorHandler(404, "User not found"));
    }
...
export const getUserById = async (req, res) => {
  try {
    const { id } = req.params;
    const user = await User.findById(id).select("_id fullname avatar");
    if (!user) {
...
export const getUserByEmail = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }).select("_id fullname avatar");
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
    res.status(200).json(user);
